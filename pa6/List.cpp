...
void List::cleanup(){
    ...
    while (current != backDummy) {
        ...
        while (runner != backDummy) {
			...
			if(runner == afterCursor){
				run_flag = 0;
			}
            if (current->data == runner->data) {
				if(runner == beforeCursor){
					beforeCursor = runner->prev;
				}
				if(run_flag == 1 && cur_flag == 0){
					//beforeCursor = runner->prev;
					pos_cursor--;
				}
				if(runner == afterCursor){
	...
	while(cur != backDummy){
		...
		while(run != backDummy){
			...
			if(run->data == cur->data){
				//delete a node
				//if the duplicate is behind the cursor decrement position before deleting
				if(run == beforeCursor){
					beforeCursor = run->prev;
				}
				if(behind_cursor == 1 && cur_after == 0){
					pos_cursor--;
				}
				if(run == afterCursor){
					afterCursor = afterCursor->next;
